...
//(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)
void LCD_myColor_Fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
...
void LCD_myColor_Fill_Window(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
		uint16_t *color, int scale) {
	uint16_t height, width, width_tt;
	uint16_t i, j;
	width_tt = ex - sx + 1;
...
// size: font size // x,y: starting coordinate
// *p: string
void LCD_ShowString_trans(uint16_t x, uint16_t y, uint16_t width, uint16_t height,
		uint8_t size, uint8_t *p) {
	uint8_t x0 = x;
	width += x;
	height += y;
